...
GoogleTokenStrategy.prototype.userProfile = function(accessToken, done) {
  // Check if the given token is an ID token (per)
  // TODO: Properly verify this by checking the JWT signature
  var isIdToken = (accessToken.match(/\./g) || []).length === 2;

  var idTokenURL = 'https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=' + accessToken;
  var accessTokenURL = 'https://www.googleapis.com/oauth2/v1/userinfo';

  this._oauth2.get(isIdToken ? idTokenURL : accessTokenURL, isIdToken ? null : accessToken, function (err, body, res) {
    ...
    try {
      ...
      profile.id = isIdToken ? json.sub : json.id; // Classic access tokens have the ID  in the "id" field, id-tokens in the "sub" field 
      profile.displayName = json.name;
      profile.token = isIdToken ? 'id-token' : 'access-token';
